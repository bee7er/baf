...
**Short Code for Displaying Folder Contents**

Use the short code [mlpp-display-files] to display files with links of a folder in the Media Library. It take one parameter, which is the ID of the folder you wish to display. You can find the folder ID either by holding the mouse over a folder in the MLPP folder tree or by viewing the folder and checking the browser address where you will see 'media-folder' followed by a number such as 'media-folder=1644'. To use the short code, include the folder ID like so: [mlpp-display-files folder_id="1644"].
...
[maxgallery-video url=”https://www.youtube.com/watch?v=video-id” width=”200″ height=”200″ cover=”path-to-video-cover-image” title=”Video Title″ caption=”Optional Caption” skin=”portal”]

These are the shortcode parameters:
url - address of the youtube video page. Do not use the embedded address or URL.
width - width of the cover image
height - height of the cover image
cover - address to the video cover image. You have to supply your own image. You can use MaxGalleria to create the cover image by adding the video to a video gallery and the cover image with be downloaded and added to your Media Library.
title - the title of the video (optional)
caption - the caption that will appear below the video (optional)
skin - CSS skin for the video. The default is 'standard'. Other skin options include:
...
For users of Wordpress Gallery MaxGalleria we have include an additional shortcode for displaying a single video. Here is an example of how to use it:

== Changelog ==
= 3.2.8 =
* Added mime type icons and a short code to view folder contents, mlpp-display-files

= 3.2.7 =
* Added the code to hide folders and to skip the scanning of non image folders

= 3.2.6 =
* Fix bug preventing selection and insertion of images into posts and pages

= 3.2.5 =
* Integrated MLPP to add a product image and product gallery images
...
= 3.2.2 =
* Added code to remove extra slashes that may be added on Windows servers and fix an issue with inserting non image files using the Add MLPP Media button
...
= 3.0.7 =
* Added support link and email to the MLPP page

= 3.0.6 =
* Fixed problem deleting a folder
...
* Added drag and drop move

= 3.0.2 =
* Added support for Wordpress multisite (Multisites created with Wordpress 3.5 or later)
* Fixed post edit page image insert popup layout error in WP 4.5.2
* Fixed problem activating the MLPP license 

= 3.0.1 =
* Added MG Video Shortcode to MLPP
* Fixed WP gallery popup layout issue

= 2.31 =
* Added button to post/page edit page to display and insert from MLP
